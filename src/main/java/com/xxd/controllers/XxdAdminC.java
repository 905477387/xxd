...
//	private XxdIntegrationSI integrationService;
	...
	@Autowired
	private XxdGoodsGroupSI goodsGroupService;
	
	@Autowired
	private XxdGoodsPriceGroupSI goodsPriceGroupService;
...
//	}
   ...
   /* @AdminLogin
    @RequestMapping("/goods_jpush_select")*/
    ...
    public ModelAndView goods_group(Integer goodsId) {
    	ModelAndView mav = new ModelAndView();
    	//判断当前商品是否已经参加了闪购活动
    	XxdGoodsFlash goodsFlash = goodsFlashService.selectByGoodId(goodsId);
    	if(null != goodsFlash) {
    		mav.setViewName(Constans.ACTIVITYHTML);
    		mav.addObject("tip", Constans.ACTIVITYFLASH);
    		return mav;
    	}
    	//判断当前商品是否已经有了拼团活动
    	XxdGoodsGroup goodsGroup = goodsGroupService.selectByGoodsId(goodsId);
    	if(null != goodsGroup) {
    		mav.setViewName("/admin/goods_group_show");
    		...
    		goodsGroup.setGroupPicture(ProperU.read(Constans.PROSOURCE, "host") + Constans.IMGHANDLER + Constans.GOODSIMGDIR + goodsGroup.getGroupPicture());
    		mav.addObject("goodsGroup", goodsGroup);
    		ArrayList<XxdGoodsPriceGroup> goodsPriceGroup = goodsPriceGroupService.selectByGoodsGroupId(goodsGroup.getId());
    		for(Integer i = 0 ; i < goodsPriceGroup.size(); i ++) {
    			goodsPriceGroup.get(i).setImg(ProperU.read(Constans.PROSOURCE, "host") + Constans.IMGHANDLER + Constans.GOODSIMGDIR + goodsPriceGroup.get(i).getImg());
        	}
    	...
    	for(Integer i = 0 ; i < xxdGoodsPrices.size(); i ++) {
    		...
    		BigDecimal price4 = price3.multiply(profit);
     ...
     */
	@AdminLogin
	@RequestMapping("/goods_flash.html")
	public ModelAndView goods_flash(Integer goodsId) {
		ModelAndView mav = new ModelAndView();
		//判断当前商品是否已经有了拼团活动
    	XxdGoodsGroup goodsGroup = goodsGroupService.selectByGoodsId(goodsId);
    	if(null != goodsGroup) {
    		mav.setViewName(Constans.ACTIVITYHTML);
    		mav.addObject("tip", Constans.ACTIVITYGROUP);
    		return mav;
    	}
		//判断当前商品是否已经参加了闪购活动
		XxdGoodsFlash goodsFlash = goodsFlashService.selectByGoodId(goodsId);
		ArrayList<XxdGoodsParametric> xxdGoodsParametric = goodsParametricService.selectByGoodsId(goodsId);
		if(goodsFlash != null) {
			mav.setViewName("/admin/goods_flash_show");
			goodsFlash.setFlashPicture(ProperU.read(Constans.PROSOURCE, "host") + Constans.IMGHANDLER + Constans.GOODSIMGDIR + goodsFlash.getFlashPicture());
			mav.addObject("goodsFlash", goodsFlash);
			mav.addObject("xxdGoodsParametric", xxdGoodsParametric);
			ArrayList<XxdGoodsPriceFlash> goodsPriceFlash = goodsPriceFlashService.selectByGoodsFlashId(goodsFlash.getId());
			for(Integer i = 0 ; i < goodsPriceFlash.size(); i ++) {
				//转换规格图片
				goodsPriceFlash.get(i).setImg(ProperU.read(Constans.PROSOURCE, "host") + Constans.IMGHANDLER + Constans.GOODSIMGDIR + goodsPriceFlash.get(i).getImg());
			}
			mav.addObject("goodsPriceFlash", goodsPriceFlash);
			return mav;
		}
		mav.setViewName("/admin/goods_flash");
		ArrayList<XxdGoodsPriceFlash> goodsPriceFlash = new ArrayList<XxdGoodsPriceFlash>();
		ArrayList<XxdGoodsPrice> xxdGoodsPrices = goodsPriceService.selectAllByGoodsId(goodsId);
		ArrayList<XxdSystem> leaderProfit = systemService.selectLikeByName("leaderProfit%");
		BigDecimal leaderProfit2 = new BigDecimal("0");
		for(XxdSystem lp : leaderProfit) {
			if(lp.getName().equals("leaderProfit2")) {
				leaderProfit2 = new BigDecimal(lp.getValue()).divide(new BigDecimal(100));
			}
		}
		for(Integer i = 0 ; i < xxdGoodsPrices.size(); i ++) {
			XxdGoodsPriceFlash goodsPriceFlashs = new XxdGoodsPriceFlash(xxdGoodsPrices.get(i));
			//转换规格图片
			goodsPriceFlashs.setImg(ProperU.read(Constans.PROSOURCE, "host") + Constans.IMGHANDLER + Constans.GOODSIMGDIR + goodsPriceFlashs.getImg());
			goodsPriceFlashs.setFlashMemberPrice(goodsPriceFlashs.getMemberPrice().subtract(goodsPriceFlashs.getMemberPrice().subtract(goodsPriceFlashs.getPrice()).multiply(leaderProfit2)));
			goodsPriceFlashs.setSign(i);
			goodsPriceFlash.add(goodsPriceFlashs);
		}
		mav.addObject("goodsId", goodsId);
		mav.addObject("xxdGoodsParametric", xxdGoodsParametric);
		mav.addObject("goodsPriceFlash", goodsPriceFlash);
    ...
    @RequestMapping("/system_notification.html")
    public ModelAndView systemNotification() {
    	ModelAndView mav = new ModelAndView();
    	mav.setViewName("/admin/system_notification");
    	return mav;
    }
    
    @AdminLogin
    @RequestMapping("/goods_jpush_select.html")
    public ModelAndView goodsJpushSelect(Integer userhopebuyId) {
    	ModelAndView mav = new ModelAndView();
    	mav.setViewName("/admin/goods_jpush_select");
    	mav.addObject("userhopebuyId", userhopebuyId);
    	return mav;
    }
