...
public interface XxdBuyOrderMapper {
    ...
    @Select({
        ...
        "id, time, total_price, addr_id, freight_id, order_id, buy_id, sta, type",
        "from xxd_buy_order",
        "where time  > date_add(now(),interval 30 minute) AND sta = 1"
    })
    @Results({
        @Result(column="id", property="id", jdbcType=JdbcType.INTEGER, id=true),
        @Result(column="time", property="time", jdbcType=JdbcType.VARCHAR),
    ...
    ArrayList<XxdBuyOrder> selectAllUnpaid();
    ...
    @Update({
        "update xxd_buy_order",
        "set total_price = total_price - #{totalPrice,jdbcType=DECIMAL},",
        "where id = #{id,jdbcType=INTEGER}"
    })
    int updateTotalPrice(XxdBuyOrder record);
}